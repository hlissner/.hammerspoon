...
local function range(reverse)
  local screens = hs.screen.allScreens()
  local x = reverse and #screens or 1
  local len = reverse and 1 or #screens
  local result = {}
  for i=x,len do
    local f = screens[i]:frame()
    table.insert(result, reverse and f.x or f.w)
  end
  return result
end

local function clampRange(range, fn)
  for i = 1,#range do
    if fn(range[i]) then
      return range[i]
    end
...
function window:resizeOut (direction, amount)
  ...
  elseif direction == self.direction.left then
    if amount == true then
      local x = clampRange(range(true), function (x) return f.x + f.w > x end)
      if f.x > x then
        f.w = f.x + x
      end
    else
  ...
  elseif direction == self.direction.right then
    if amount == true then
      f.w = f.x + clampRange(range(), function (x) return f.x + f.w < x end)
    else
      f.w = f.w + amount
    end
  end
